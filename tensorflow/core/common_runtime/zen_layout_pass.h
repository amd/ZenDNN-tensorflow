...
enum class zenTensorType { QINT8 = 0, QUINT8 = 1, FLOAT = 2, BFLOAT16 = 3 };
...
class ZenMemoryPoolBase {
 public:
  ...
  static ZenMemoryPoolBase *zenMemPoolArr[ZEN_MEM_POOL_LIMIT];
...
class ZenMemoryPool : public ZenMemoryPoolBase {
 ...
 public:
  ...
  static ZenMemoryPool *getZenMemPool(int index) {
    ...
    mtx.unlock();
    return static_cast<ZenMemoryPool<T> *>(zenMemPoolArr[index]);
  }
  ...
  int acquireZenPoolTensor(OpKernelContext *context, Tensor **output,
    ...
    */
    ...
    if (!acquire_flag) {
      ...
      DataType macro_value = (type == zenTensorType::QINT8)
                                 ? DT_QINT8
                                 : (type == zenTensorType::QUINT8)
                                       ? DT_QUINT8
                                       : (type == zenTensorType::BFLOAT16)
                                             ? DT_BFLOAT16
                                             : DT_FLOAT;
      zenTensorPoolArr[poolOffset].zenType = type;
      context->allocate_temp(macro_value, shape,
      ...
      *output = (zenTensorPoolArr[poolOffset].zenTensorPtr);
      ...
      zendnnInfo(ZENDNN_FWKLOG, "TF-MEM-POOL: Acquired TensorPool Ptr[",
                 poolOffset, "] pointed to size(no. of elements)",
