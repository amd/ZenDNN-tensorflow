...
void zenVitisAIConvolution2DBiasRelu(
    ...
    int intermediate_float_scale, void *bias_array, vector<float> &scale,
    void *output, bool Tinput, bool Toutput, vector<float> &bias_scale,
...
template <typename Tinput, typename Tfilter, typename Tbias, typename Toutput,
          bool is_sum, bool is_depthwise = false, bool bias_enabled = true,
          typename Tsum = quint8>
class ZenVitisAIConv2DOp : public OpKernel {
 public:
  explicit ZenVitisAIConv2DOp(OpKernelConstruction *context)
    ...
    if (context->HasAttr("explicit_paddings")) {
      // explicit_paddings_control = true;
      OP_REQUIRES_OK(
    ...
    if (context->HasAttr("bias_scale")) {
      OP_REQUIRES_OK(context, context->GetAttr("bias_scale", &bias_scale));
    }
    OP_REQUIRES_OK(context, context->GetAttr("out_scale", &output_scale));

    OP_REQUIRES_OK(context, context->GetAttr("is_relu", &is_relu));
    if (context->HasAttr("sum_scale")) {
      OP_REQUIRES_OK(context, context->GetAttr("sum_scale", &sum_scale));
      OP_REQUIRES_OK(context,
                     context->GetAttr("add_out_scale", &add_out_scale));
    }
    if (context->HasAttr("intermediate_float_scale")) {
      OP_REQUIRES_OK(context, context->GetAttr("intermediate_float_scale",
                                               &intermediate_float_scale));
    }
    if (context->HasAttr("relu_alpha")) {
      OP_REQUIRES_OK(context, context->GetAttr("relu_alpha", &relu_alpha));
    }
  }
  void Compute(OpKernelContext *context) override {
    ZenExecutor *ex = ex->getInstance();
    ...
    if (bias_enabled) {
      OP_REQUIRES_OK(context, InitBiasAddArgs(context, &bias_add_args));
      bias_array = const_cast<Tbias *>(bias_add_args.bias_add_data);
    }
    ...
    GetWindowedOutputSizeVerboseV2(height, kernel_height, dilation_rows,
                                   stride_rows, params_.padding, &out_rows,
                                   &pad_rows_before, &pad_rows_after);
    for (auto const &padding_val : explicit_paddings_) {
      if (padding_val > 0) {
        pad_rows_before = pad_rows_after = pad_cols_before = pad_cols_after =
            padding_val;
        out_rows = out_cols =
    ...
    size_t depth = 1;
    std::vector<float> scales(depth);
    scales[0] = std::pow(2, -input_scale - filter_scale + output_scale);

    std::vector<float> bias_scales(depth);
    if (bias_enabled) {
      bias_scales[0] = std::pow(2, input_scale + filter_scale);
    }

    bool is_both_float = (std::is_same<Tinput, float>::value) &&
                         (std::is_same<Toutput, float>::value);
    bool is_input_float = std::is_same<Tinput, float>::value;
    bool is_output_float = std::is_same<Toutput, float>::value;
    bool is_input_quint8 = std::is_same<Tinput, quint8>::value;
    bool is_output_quint8 = std::is_same<Toutput, quint8>::value;
    ...
    if (is_sum) {
      ...
      OP_REQUIRES_OK(
          context, add_tensor.BitcastFrom(
                       add_tensor, is_output_quint8 == 1 ? DT_QUINT8 : DT_QINT8,
                       add_tensor.shape()));
      context->set_output(0, add_tensor);
      output = context->mutable_output(0);
      if (zenPoolEnable) {
        ...
        if (zenPoolBuffer) {
          ...
          if (out_type == zenTensorType::QINT8) {
            output_array = const_cast<qint8 *>(output->flat<qint8>().data());
            zenPoolBuffer->zenMemPoolUpdateTensorPtrStatus(
                context, (qint8 *)output_array, out_links, reset);
          } else if (out_type == zenTensorType::QUINT8) {
            output_array = const_cast<quint8 *>(output->flat<quint8>().data());
            zenPoolBuffer->zenMemPoolUpdateTensorPtrStatus(
                context, (quint8 *)output_array, out_links, reset);
          }
        }
      }
    } else {
    ...
    zendnnInfo(ZENDNN_FWKLOG, "ZenVitisAIConv2DOp: in_depth = ", channels,
               ", input_cols = ", height, ", filter_cols = ", kernel_width,
               ", input_rows = ", width, ", filter_rows = ", kernel_height,
               ", stride_rows = ", stride_rows, ", stride_cols = ", stride_cols,
               ", dilation_rows = ", dilation_rows,
               ", dilation_cols = ", dilation_cols,
               ", out_depth = ", output_channels, ", out_rows = ", out_rows,
               ", out_cols = ", out_cols);

    // For specific layers, where Add is a post-op and the sum
    // is of qint8, the output buffer should be of qint8.
    if (is_sum && !std::is_same<Tsum, quint8>::value) {
      is_output_quint8 = 0;
    }

    primitive_attr conv_attr;
    if (!is_both_float) {
      zenVitisAIConvolution2DBiasRelu(
          eng, s, conv_attr, context,
          ...
          /*pad_cols_before*/ pad_cols_before,
          /*pad_rows_after*/ pad_rows_after,
          /*pad_cols_after*/ pad_cols_after,
          ...
          /*add_out_scale*/ add_out_scale,
          /*intermediate_float_scale*/ intermediate_float_scale,
          /*bias_array*/ bias_array,
          /*scales*/ scales,
          /*output_array*/ output_array,
          /*in_type*/ is_input_quint8,
          /*out_type*/ is_output_quint8,
          /*bias_scales*/ bias_scales,
          /*is_relu*/ is_relu,
          /*relu_alpha*/ relu_alpha,
          /*is_sum*/ is_sum,
          /*bias_enabled*/ bias_enabled,
          /*is_input_float*/ is_input_float,
          /*is_output_float*/ is_output_float,
          /*is_depthwise*/ is_depthwise,
          /*cached_filter_data_*/ &cached_filter_data_,
          /*reset*/ reset);
    } else {
      primitive_attr conv_attr;
      if (is_relu) {
        post_ops ops;
        ops.append_eltwise(1.0f, zendnn::algorithm::eltwise_relu, 0.0f, 0.0f);
        conv_attr.set_post_ops(ops);
      }
      zenConvolution2DBiasOrRelu(
          eng, s, conv_attr, input_array, batch_size, channels, height, width,
          filter_array, output_channels, kernel_height, kernel_width,
          pad_rows_before, pad_cols_before, pad_rows_after, pad_cols_after,
          stride_rows, stride_cols, bias_array, output_array, out_rows,
          out_cols, reorder_before, reorder_after, &cached_filter_data_,
          is_input_float, context);
    }
    if (zenEnvObj.zenEnableMemPool && zenPoolBuffer) {
      zenPoolBuffer->zenMemPoolFree(context, (void *)input_array);
    }
 ...
 private:
  ...
  bool reorder_before, reorder_after, reset;
  int in_links, out_links;
  int input_scale, filter_scale, bias_scale, sum_scale, output_scale,
      add_out_scale;
  int intermediate_float_scale;
  bool is_relu;
  float relu_alpha;
  // Additional attributes to support new Padding definition and Persistent
  // tensors
  std::vector<int64> explicit_paddings_;
  Tensor cached_filter_data_ GUARDED_BY(mu_);
  Tensor cached_data_ GUARDED_BY(mu_);
};
      ...
      case FusedComputationType::kBiasAdd: {
        ...
        if (is_depthwise) {
          ...
          zenConvolution2DBiasOrRelu(
              ...
              dimensions.stride_cols, bias_arr, output_array,
              dimensions.out_rows, dimensions.out_cols, reorder_before,
              reorder_after, cached_filter_data_, is_input_float, context);
        } else {
      ...
      if (zenEnableMemPool) {
        ...
        if (zenPoolBuffer) {
          const float *output_array =
              const_cast<float *>(output->flat<float>().data());
          zenPoolBuffer->zenMemPoolUpdateTensorPtrStatus(
              context, (float *)output_array, out_links, reset);
        }
      }
    } else if (!is_float) {
      // Support for Persistent tensor (for BFLOAT16)
      int res = cached_data_.NumElements();
      if (res <= 0) {
        context->allocate_temp(DT_BFLOAT16, out_shape, &cached_data_);
      }
      output = &cached_data_;
...
REGISTER_KERNEL_BUILDER(Name("VitisAIDepthwiseConv2D").Device(DEVICE_CPU),
                        NoOp);

REGISTER_KERNEL_BUILDER(Name("VitisAIConv2DWithoutBias").Device(DEVICE_CPU),
                        NoOp)

// Default all float inputs
REGISTER_KERNEL_BUILDER(
    Name("ZenVitisAIConv2D")
        .Device(DEVICE_CPU)
        .TypeConstraint<float>("Tinput")
        .TypeConstraint<float>("Toutput"),
    ZenVitisAIConv2DOp<float, float, float, float, false, false>);
REGISTER_KERNEL_BUILDER(
    Name("ZenVitisAIConv2DWithSum")
        .Device(DEVICE_CPU)
        .TypeConstraint<float>("Tinput")
        .TypeConstraint<float>("Toutput"),
    ZenVitisAIConv2DOp<float, float, float, float, true, false>);

// For float inputs quint8 output
REGISTER_KERNEL_BUILDER(
    Name("ZenVitisAIConv2D")
        .Device(DEVICE_CPU)
        .TypeConstraint<float>("Tinput")
        .TypeConstraint<quint8>("Toutput"),
    ZenVitisAIConv2DOp<float, float, float, quint8, false, false>);
REGISTER_KERNEL_BUILDER(
    Name("ZenVitisAIConv2DWithSum")
        .Device(DEVICE_CPU)
        .TypeConstraint<float>("Tinput")
        .TypeConstraint<quint8>("Toutput"),
    ZenVitisAIConv2DOp<float, float, float, quint8, true, false>);

// For u8 inputs u8 outputs
REGISTER_KERNEL_BUILDER(
    Name("ZenVitisAIConv2D")
        .Device(DEVICE_CPU)
        .TypeConstraint<quint8>("Tinput")
        .TypeConstraint<quint8>("Toutput"),
    ZenVitisAIConv2DOp<quint8, float, float, quint8, false, false>);
REGISTER_KERNEL_BUILDER(Name("ZenVitisAIConv2DWithSum")
                            .Device(DEVICE_CPU)
                            .TypeConstraint<quint8>("Tinput")
                            .TypeConstraint<quint8>("Toutput")
                            .TypeConstraint<quint8>("Tsum"),
                        ZenVitisAIConv2DOp<quint8, float, float, quint8, true,
                                           false, true, quint8>);
...
REGISTER_KERNEL_BUILDER(
    Name("ZenVitisAIConv2DWithSum")
        ...
        .TypeConstraint<quint8>("Tinput")
        .TypeConstraint<quint8>("Toutput")
        .TypeConstraint<qint8>("Tsum"),
    ZenVitisAIConv2DOp<quint8, float, float, quint8, true, false, true, qint8>);

// SPECIAL CASE WITHOUT BIAS
REGISTER_KERNEL_BUILDER(
    Name("ZenVitisAIConv2DWithoutBias")
        .Device(DEVICE_CPU)
        .TypeConstraint<quint8>("Tinput")
        .TypeConstraint<quint8>("Toutput"),
    ZenVitisAIConv2DOp<quint8, float, float, quint8, false, false, false>);

// for u8 inputs s8 outputs
REGISTER_KERNEL_BUILDER(
    Name("ZenVitisAIConv2D")
        .Device(DEVICE_CPU)
        .TypeConstraint<quint8>("Tinput")
        .TypeConstraint<qint8>("Toutput"),
    ZenVitisAIConv2DOp<quint8, float, float, qint8, false, false>);
REGISTER_KERNEL_BUILDER(
    Name("ZenVitisAIConv2DWithSum")
        .Device(DEVICE_CPU)
        .TypeConstraint<quint8>("Tinput")
        .TypeConstraint<qint8>("Toutput"),
    ZenVitisAIConv2DOp<quint8, float, float, qint8, true, false>);

// For s8 inputs and u8 outputs
REGISTER_KERNEL_BUILDER(
    Name("ZenVitisAIConv2D")
        .Device(DEVICE_CPU)
        .TypeConstraint<qint8>("Tinput")
        .TypeConstraint<quint8>("Toutput"),
    ZenVitisAIConv2DOp<qint8, float, float, quint8, false, false>);
REGISTER_KERNEL_BUILDER(
    Name("ZenVitisAIConv2DWithSum")
        .Device(DEVICE_CPU)
        .TypeConstraint<qint8>("Tinput")
        .TypeConstraint<quint8>("Toutput"),
    ZenVitisAIConv2DOp<qint8, float, float, quint8, true, false>);

// For s8 inputs and s8 outputs
REGISTER_KERNEL_BUILDER(
    Name("ZenVitisAIConv2D")
        .Device(DEVICE_CPU)
        .TypeConstraint<qint8>("Tinput")
        .TypeConstraint<qint8>("Toutput"),
    ZenVitisAIConv2DOp<qint8, float, float, qint8, false, false>);
REGISTER_KERNEL_BUILDER(
    Name("ZenVitisAIConv2DWithSum")
        .Device(DEVICE_CPU)
        .TypeConstraint<qint8>("Tinput")
        .TypeConstraint<qint8>("Toutput"),
    ZenVitisAIConv2DOp<qint8, float, float, qint8, true, false>);

// Special Case for VGG with float outputs
REGISTER_KERNEL_BUILDER(
    Name("ZenVitisAIConv2D")
        .Device(DEVICE_CPU)
        .TypeConstraint<quint8>("Tinput")
        .TypeConstraint<float>("Toutput"),
    ZenVitisAIConv2DOp<quint8, float, float, float, false, false>);

REGISTER_KERNEL_BUILDER(
    Name("ZenVitisAIDepthwiseConv2D")
        .Device(DEVICE_CPU)
        .TypeConstraint<quint8>("Tinput")
        .TypeConstraint<quint8>("Toutput"),
    ZenVitisAIConv2DOp<quint8, float, float, quint8, false, true>);
