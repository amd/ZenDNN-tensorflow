...
class ZenVitisAIAvgQuantizedPoolOp : public OpKernel {
 private:
  std::vector<int32> ksize_;
  std::vector<int32> stride_;
  Padding padding_;
 ...
 public:
  explicit ZenVitisAIAvgQuantizedPoolOp(OpKernelConstruction *context)
      : OpKernel(context) {
    OP_REQUIRES_OK(context, context->GetAttr("ksize", &ksize_));
    OP_REQUIRES(
        context, ksize_.size() == 4,
...
REGISTER_KERNEL_BUILDER(Name("ZenVitisAIAvgPool")
                        ...
                        ZenVitisAIAvgQuantizedPoolOp<qint8, qint8>);
REGISTER_KERNEL_BUILDER(Name("ZenVitisAIAvgPool")
                            .Device(DEVICE_CPU)
                            .TypeConstraint<quint8>("Tinput")
                            .TypeConstraint<quint8>("Toutput"),
                        ZenVitisAIAvgQuantizedPoolOp<quint8, quint8>);

REGISTER_KERNEL_BUILDER(Name("ZenVitisAIAvgPool")
                            .Device(DEVICE_CPU)
                            .TypeConstraint<quint8>("Tinput")
                            .TypeConstraint<float>("Toutput"),
                        ZenVitisAIAvgQuantizedPoolOp<quint8, float>);
}  // namespace tensorflow
