...
class ZenMaxQuantizedPoolOp : public OpKernel {
 private:
 ...
 public:
  explicit ZenMaxQuantizedPoolOp(OpKernelConstruction *context)
      : OpKernel(context) {
...
class ZenVitisAIMaxPoolOp : public OpKernel {
 private:
  std::vector<int32> ksize_;
  std::vector<int32> stride_;
  Padding padding_;
 ...
 public:
  explicit ZenVitisAIMaxPoolOp(OpKernelConstruction *context)
      : OpKernel(context) {
    OP_REQUIRES_OK(context, context->GetAttr("ksize", &ksize_));
    OP_REQUIRES(
        context, ksize_.size() == 4,
  ...
  void Compute(OpKernelContext *context) override {
    const Tensor &input = context->input(0);
    auto input_map = input.tensor<Tinput, 4>();  // experimented and proven that
                                                 // it is row-major
    const Tinput *input_array = input_map.data();
    ...
    zendnn::memory pool_src_memory, pool_dst_memory, pool_dst_float_memory;
    pool_src_memory =
        memory({{pool_src_tz}, dt::u8, tag::acdb}, eng, (quint8 *)input_array);
    if (out_type != zenTensorType::FLOAT) {
      pool_dst_memory = memory({{pool_dst_tz}, dt::u8, tag::acdb}, eng,
                               (quint8 *)output_array);
    } else {
      pool_dst_memory = memory({{pool_dst_tz}, dt::u8, tag::acdb}, eng);
      pool_dst_float_memory =
          memory({{pool_dst_tz}, dt::f32, tag::acdb}, eng, output_array);
    }
    ...
    if (out_type == zenTensorType::FLOAT) {
      ...
      // scales[0] = std::pow(2, 2);
      // reorder_attr.set_output_scales(0 , scales);

      // net.push_back(reorder(pool_dst_memory, pool_dst_float_memory,
      // reorder_attr));
      net.push_back(reorder(pool_dst_memory, pool_dst_float_memory));
      net_args.push_back({{ZENDNN_ARG_SRC, pool_dst_memory},
                          {ZENDNN_ARG_DST, pool_dst_float_memory}});
    }
    for (size_t i = 0; i < net.size(); ++i) {
      net.at(i).execute(s, net_args.at(i));
    }
...
template <typename T>
class ZenMaxPoolOp : public OpKernel {
 private:
  std::vector<int32> ksize_;
  std::vector<int32> stride_;
  Padding padding_;
 ...
 public:
  explicit ZenMaxPoolOp(OpKernelConstruction *context) : OpKernel(context) {
    OP_REQUIRES_OK(context, context->GetAttr("ksize", &ksize_));
    OP_REQUIRES(
        context, ksize_.size() == 4,
        errors::InvalidArgument("Kernel size field must specify 4 dimensions"));
  ...
  void Compute(OpKernelContext *context) override {
    ...
    auto is_input_float = std::is_same<T, float>::value;
    ...
    if ((blocked || !is_input_float)) {
      zendnnInfo(
          ZENDNN_FWKLOG,
          "zenMaxPool (TF kernel): New API for BLOCKED MAXPOOL zenMaxPool");
      using tag = memory::format_tag;
      using dt = memory::data_type;
      ZenExecutor *ex = ex->getInstance();
      engine eng = ex->getEngine();
      ...
      zendnnInfo(ZENDNN_FWKLOG,
                 "zenMaxPool (TF kernel): New API for NHWC MAXPOOL zenMaxPool");
      max_pooling((float *)input_array, params.tensor_in_batch, params.depth,
                  params.tensor_in_rows, params.tensor_in_cols,
...
REGISTER_KERNEL_BUILDER(Name("ZenVitisAIMaxPool")
                        ...
                        ZenVitisAIMaxPoolOp<qint8, qint8>);
REGISTER_KERNEL_BUILDER(Name("ZenVitisAIMaxPool")
                            .Device(DEVICE_CPU)
                            .TypeConstraint<quint8>("Tinput")
                            .TypeConstraint<quint8>("Toutput"),
                        ZenVitisAIMaxPoolOp<quint8, quint8>);

REGISTER_KERNEL_BUILDER(Name("ZenVitisAIMaxPool")
                            .Device(DEVICE_CPU)
                            .TypeConstraint<qint8>("Tinput")
                            .TypeConstraint<float>("Toutput"),
                        ZenVitisAIMaxPoolOp<qint8, float>);
REGISTER_KERNEL_BUILDER(Name("ZenVitisAIMaxPool")
                            .Device(DEVICE_CPU)
                            .TypeConstraint<quint8>("Tinput")
                            .TypeConstraint<float>("Toutput"),
                        ZenVitisAIMaxPoolOp<quint8, float>);
