...
// --------------------------------------------------------------------------
template <typename T, AxisArgumentName AxisArgName, bool vitisop = false>
...
template <typename T>
using ZenVitisAIConcatV2Op = ZenConcatBaseOp<T, NAME_IS_AXIS, true>;
...
REGISTER_KERNEL_BUILDER(Name("_ZenVitisAIConcatV2")
                            .Device(DEVICE_CPU)
                            .TypeConstraint<quint8>("T")
                            .HostMemory("axis"),
                        ZenVitisAIConcatV2Op<quint8>);
