...
class ZenFusedInceptionOp : public OpKernel {
 private:
  ...
  Status InitFusedBatchNormArgs(
      OpKernelContext *context, int start_index, float **bn_mean,
      float **bn_offset,
  ...
  void run_convolution(OpKernelContext *context, zendnn::engine eng,
    ...
    switch (fused_computation_) {
      ...
      case FusedComputationType::kFusedBatchNormWithRelu:
        ...
        for (int r = 0; r < output_channels; r++) {
          bias_from_bn[r] =
              bn_offset[r] - (scaling_factor.data()[r] * bn_mean[r]);
        }
        bias_array = bias_from_bn;
        is_bn_enabled = true;
        is_relu_enabled = true;
    ...
    if (res <= 0) {
      // Weight are modified in case of BatchNormConv Fusion.
      // Weights are multiplied with scale across output channel.
      // and then modified wieghts are cached to avoid this computation.
      // ConvBNFusion(input,weight,bias,scale) = scale*Conv(input,weight,bias)
      // scale*Conv(input,weight,bias) = Conv(input,scale*weight,bias)
      if (is_bn_enabled) {
        size_t kernel_patch = channels * kernel_h * kernel_w;
#pragma omp parallel for num_threads(zenEnvObj.omp_num_threads)
        for (size_t i = 0; i < kernel_patch; ++i) {
          for (size_t ii = 0; ii < output_channels; ++ii)
            filter_array[i * output_channels + ii] =
                filter_array[i * output_channels + ii] *
                scaling_factor.data()[ii];
        }
      }
    ...
    if (res <= 0 && (format != tag::nhwc || is_bn_enabled)) {
